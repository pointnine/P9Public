...
class CircleClient(object):
    ...
    def __init__(self, host, fund, username, password):
        ...
        handler = urllib.HTTPBasicAuthHandler(password_mgr=pwd_mgr)
    ...
    def run_raw(self, workflow, action, **kwargs):
        ...
        request = urllib.Request(URL, data=data.encode('ascii'), headers=self.HEADERS)
        return self.opener.open(request).read().decode('ascii')
